...
function Dashboard() {
  ...
  const [averageHandleTime, setAverageHandleTime] = useState<number>(0);
  const [firstContactResolutionRate, setFirstContactResolutionRate] = useState<number>(0);
  const [customerSatisfaction, setCustomerSatisfaction] = useState<number>(0);
  const [queueInfo, setQueueInfo] = useState<{ length: number, waitTime: number }>({ length: 0, waitTime: 0 });
